...
def train_model_with_curriculum(
    model_setup: ModelSetup,
    train_graphs: list,  # Pass raw list of graphs instead of loader
    val_loader: DataLoader,
    test_loader: DataLoader,
    optimizer: torch.optim.Optimizer,
    scheduler: torch.optim.lr_scheduler._LRScheduler,
    ...
    """Run curriculum-based training with progressive difficulty.

    Args:
        model_setup: Model setup containing model and scalers
        train_graphs: List of training graphs (not a DataLoader)
        val_loader: Validation data loader
        test_loader: Test data loader
        optimizer: Optimizer for training
        scheduler: Learning rate scheduler
        ...
        model_cfg: Model configuration for checkpointing
        collate_fn: Collate function for creating data loaders

    Returns:
        TrainingState: Final training state with best metrics
    """
    from .curriculum import create_curriculum_loaders, CurriculumScheduler, get_default_curriculum_stages

    # Setup curriculum thresholds
    if config.curriculum_thresholds is None:
        thresholds = get_default_curriculum_stages(len(train_graphs), config.curriculum_stages)
    else:
        thresholds = config.curriculum_thresholds
    ...
    print(f"Difficulty thresholds (node count): {thresholds}")

    # Create curriculum loaders
    curriculum_loaders, val_loader, test_loader = create_curriculum_loaders(
        train_graphs,
        val_loader,
        test_loader,
        thresholds,
    ...
    # Setup epochs per stage
    if config.curriculum_epochs is None:
        # Distribute epochs equally across stages
        base_epochs = config.epochs // config.curriculum_stages
        remainder = config.epochs % config.curriculum_stages
        epochs_per_stage = [base_epochs] * config.curriculum_stages
        # Add remainder to last stage
        epochs_per_stage[-1] += remainder
    else:
        epochs_per_stage = config.curriculum_epochs
    ...
    print(f"\nStarting curriculum training with loss type: {config.loss_type.value}\n")

    # Train through all curriculum stages
    while True:
        # Get current stage info
        stage_info = curriculum_scheduler.get_current_stage_info()
        current_loader = curriculum_scheduler.get_current_loader()
        ...
        # Log metrics to TensorBoard
        logging.log_epoch_metrics(writer, global_epoch, tr_metrics, val_metrics, current_lr)

        # Log curriculum stage info
        writer.add_scalar('curriculum/stage', stage_info['stage'], global_epoch)
        writer.add_scalar('curriculum/num_graphs', stage_info['num_graphs'], global_epoch)

        # Console logging with curriculum info
        stage_prefix = f"[Stage {stage_info['stage']}/{stage_info['total_stages']} | Epoch {stage_info['epoch_in_stage']}/{stage_info['epochs_per_stage']} | Graphs: {stage_info['num_graphs']}] "

        base_log = (f"Epoch {global_epoch:03d} | "
                    f"train_tot={tr_loss:.3e} train_mse={tr_mse:.3e} train_phys={tr_phys:.3e} train_rmse={tr_rmse:.3e} train_relerr={tr_rel_error:.3e}")
        ...
        if tr_phys_metrics is not None:
            physics_log = f" | {tr_phys_metrics}"
            print(stage_prefix + base_log + physics_log)
        else:
            print(stage_prefix + base_log)

        # Model saving and early stopping
        if training_state.update_best(val_loss, global_epoch):
            writer.add_scalar('best_model/epoch', global_epoch, global_epoch)
            writer.add_scalar('best_model/val_loss', val_loss, global_epoch)
            writer.add_scalar('best_model/val_mse', val_mse, global_epoch)
            writer.add_scalar('curriculum/best_stage', stage_info['stage'], global_epoch)

            utils.save_checkpoint(
                model_setup, model_cfg, optimizer, scheduler,
                global_epoch, val_loss, val_mse, config.model_save_path
            )
        else:
            if training_state.should_stop(config.patience):
                print(f"\nEarly stopping at epoch {global_epoch} (stage {stage_info['stage']}). "
                      f"Best validation loss: {training_state.best_val_loss:.4e} "
                      f"at epoch {training_state.best_epoch}")
                writer.add_text('training/early_stopping',
                                f"Stopped at epoch {global_epoch}, best at {training_state.best_epoch}")
                break

        # Advance curriculum
        should_continue = curriculum_scheduler.step()
        if not should_continue:
            print(f"\nCurriculum completed! All {config.curriculum_stages} stages finished.")
            break

    print("\nCurriculum training completed!")
    writer.add_text('training/summary',
                    f"Curriculum training completed. Best validation loss: {training_state.best_val_loss:.4f} at epoch {training_state.best_epoch}")
