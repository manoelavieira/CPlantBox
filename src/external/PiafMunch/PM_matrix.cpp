...
CooMatrix SpUnit_matrix::toCooMatrix() {
    CooMatrix out;
    ...
    const int* temp_ni  = ij_ - 1;
    const int* temp_cmi = temp_ni + m + n;
    const int* temp_cmv = temp_cmi + nnz + nnz;

    const int* base = ij_ - 1;
    const int* ni = base;
    const int* cmi = ni + n + m;
    const int* cmv = cmi + nnz + nnz;

    int ks = 0;
    for (int j = 1; j <= n; ++j) {
        const int cnt = temp_ni[j];
        for (int t = 1; t <= cnt; ++t) {
            ++ks;
            const int i0 = temp_cmi[ks] - 1;
            const int j0 = j - 1;
            const double v = (temp_cmv[ks] > 0) ? 1.0 : -1.0;

			out.row.push_back(i0);
            out.col.push_back(j0);
            out.val.push_back(v);
        }
    }
